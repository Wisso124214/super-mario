let isJumping = false;
let isDoubleJumpingAllowed = true;
let isDoubleJumpingCooldown = false;
const doubleJumpingCooldown = 400;
let isMarioStomp = false;

function createPlayer(game, isDoubleJumpAllowed) {
  // Draw player
  ...
  isDoubleJumpingAllowed = isDoubleJumpAllowed;
}
...
function updatePlayer(delta) {
  ...
  const isJumpAllowed = isDoubleJumpingAllowed
    ? (player.body.touching.down || !isJumping) && !isDoubleJumpingCooldown
    : player.body.touching.down;

  if ((controlKeys.JUMP.isDown || this.joyStick.up) && isJumpAllowed) {
    isDoubleJumpingCooldown = true;

    setTimeout(() => {
      isDoubleJumpingCooldown = false;
    }, doubleJumpingCooldown);
  ...
  if ((controlKeys.DOWN.isDown || this.joyStick.down) && !player.body.touching.down && !isMarioStomp) {
    isMarioStomp = true;

    playerState > 0 && (controlKeys.JUMP.isDown || this.joyStick.up)
      ? player.setVelocityY(velocityY / 1.25)
      : player.setVelocityY(velocityY);
  }
  ...
  if (playerState == 2 && controlKeys.FIRE.isDown && !fireInCooldown) {
