...
const levelGravity = velocityY * gameConfig.gravity;
...
const lengthLevel = gameConfig.lengthLevel < 7 ? 7 : gameConfig.lengthLevel;

const worldWidth = screenWidth * lengthLevel;
...
let playerState = gameConfig.playerState;
...
function preload() {
  ...
  isLevelOverworld = gameConfig.isOverworld !== undefined ? gameConfig.isOverworld : Phaser.Math.Between(0, 100) <= 84;
...
function create() {
  ...
  createPlayer(this, gameConfig.doubleJump);
...
function createControls() {
  ...
  //   if (!this.settingsMenuOpen) showSettings.call(this);
  //   else hideSettings.call(this);
  // });
}
...
function generateLevel() {
  ...
  this.physics.add.collider(
    ...
    startUndergroundLevel,
    null,
    this
  );

  this.undergroundStartScreenTrigger = this.add
    .tileSprite(
      screenWidth,
      screenHeight - platformHeight,
      32,
    ...
    .setScale(screenHeight / 345)
    .setOrigin(1, 2)
  this.undergroundStartScreenTrigger.depth = 4;
  this.physics.add.existing(this.undergroundStartScreenTrigger);
  this.undergroundStartScreenTrigger.body.allowGravity = false;
  this.undergroundStartScreenTrigger.body.immovable = true;
  this.physics.add.collider(
    player,
    this.undergroundStartScreenTrigger,
    startOverworldLevel,
    null,
    this
  );
...
function startUndergroundLevel(player, trigger) {
  isLevelOverworld = false;
  this.powerDownSound.play();
  startLevel(player, trigger, this);
}

function startOverworldLevel(player, trigger) {
  isLevelOverworld = true;
  startLevel(player, trigger, this);
}

function startLevel(player, trigger, game) {
  if (!player.body.blocked.right && !trigger.body.blocked.left) return;

  game.powerDownSound.play();

  game.physics.world.setBounds(screenWidth, 0, worldWidth, screenHeight);

  applyPlayerInvulnerability.call(game, 4000);
  ...
  game.cameras.main.fadeOut(900, 0, 0, 0);

  game.hereWeGoSound.play();

  setTimeout(() => {
    if (!isLevelOverworld) {
      player.y = screenHeight / 5;
      game.musicTheme.stop();
      game.undergroundMusicTheme.play({ loop: -1 });
    }

    player.x = screenWidth * 1.1;
    game.cameras.main.pan(screenWidth * 1.5, 0, 0);
    playerBlocked = false;
    game.cameras.main.fadeIn(500, 0, 0, 0);
    createHUD.call(game);
    updateTimer.call(game);
    game.startScreenTrigger.destroy();
    game.undergroundStartScreenTrigger.destroy();
    levelStarted = true;
    if (game.settingsMenuOpen) hideSettings.call(game);
  }, 1100);
}
